---
title: 모모스 커피 (MOMOS COFFEE)
post_date: 2026-08-19T16:01:00+09:00
overview_category:
  - 전시·컨벤션
client: ''
location: 벡스코
cover_image: /assets/img/portfolio/eqb9y25xza.webp
gallery:
  - /assets/img/portfolio/7u3sn07m1r.webp
---

## MOMOS COFFEE 부스 — 벡스코에 수평선을 놓았습니다

벡스코 MOMOS COFFEE 현장 부스에 P2 LED로 초와이트 스크린을 설치 했습니다. 

***

## 배경을 세운 게 아니라, 바다를 놓았습니다

부스에 세운 화면 앞에 **선베드**가 놓여 있습니다. 그 순간 이 LED는 더 이상 '브랜드 로고가 나오는 스크린'이 아닙니다.

관람객은 화면을 **보러** 오지 않고, 화면 앞에 **누우러** 옵니다. 사진을 찍고, 그 사진에는 부스가 아니라 **바다가 찍힙니다.**

이 순간  LED는 정보를 띄우는 장치가 아니라 **공간을 만드는 재료**여야 합니다.

***

## 1. 왜 초와이드였나? - 화면비가 곧 콘셉트였습니다

수평선은 **가로로 길고 높이가 낮은 화면**에서만 성립합니다.

일반적인 16:9 화면에 바다 영상을 넣으면 하늘이 위로 잘리고 화면 가장자리에 부스 벽이 그대로 드러납니다. 관람객의 시야에 "화면의 끝"이 보이는 순간, 공간의 환상은 깨집니다.

가로로 길게 뻗은 초와이드 화면은 **시야각 전체를 화면으로 채웁니다.**

- 화면의 좌우 끝이 관람객의 주변 시야 밖으로 밀려남
- 구름과 수평선이 **잘리지 않고 끝까지 이어짐**
- 선베드에 앉은 눈높이에서 바라볼 때 실제 해변의 시야와 일치

**콘텐츠에 화면을 맞춘 게 아니라, 콘텐츠가 성립하는 화면비를 먼저 설계했습니다.**

***

### 2. 왜 P2였나? - 관람객이 화면을 만질 수 있는 거리에 있습니다

무대 LED는 관객이 최소 5m 밖에 있습니다. **전시 부스는 다릅니다. 관람객이 화면에서 1m 앞에 섭니다.**

P2는 픽셀 간격이 2mm로, **최적 시청 거리가 약 2m부터** 시작합니다. 바로 앞에 서서 봐도 격자가 보이지 않는 이 수치가, 부스 LED에서는 타협 불가 조건입니다.

- **선베드에 누운 거리(1\~2m)에서도** 픽셀 격자 없이 실사 질감 유지
- 하늘의 파랑에서 흰 구름으로 넘어가는 **미세 그라데이션에 밴딩 없음**
- 관람객이 찍는 스마트폰 사진에 **모아레 무늬가 남지 않음**

**사진 찍는 공간에서 LED에 모아레가 발생하면 홍보 효과가 현저히 저하 됩니다.**

***

## 3. 전시장 조명 아래에서 색이 살아 있어야 합니다

전시장은 무대와 정반대의 환경입니다. **천장 조명이 하루 종일 최대로 켜져 있습니다.** 밝은 백색광 아래에서 파랑은 가장 먼저 색이 빠지는 색입니다.

- 고조도 환경에 맞춘 휘도·감마 세팅으로 **하늘색의 채도 유지**
- 브랜드 아이덴티티 컬러 기준 캘리브레이션 — 인쇄물·굿즈와 색이 어긋나지 않도록
- 전시 기간 내내 **장시간 연속 구동에도 색 편차·패널 편차 없이** 균일 유지

며칠간 이어지는 전시에서 **첫날과 마지막 날의 화면이 같아야** 합니다.

***

## 4. 목공 배경과 대형 출력물, 다시 계산해 보세요

부스 배경을 대형 출력 그래픽으로 제작하면,

- 이미지는 **끝까지 고정**입니다. 시간대별 연출도, 신제품 컷 교체도 불가능합니다.
- 전시가 끝나면 **전량 폐기물**이 됩니다.
- 조명이 닿는 각도에 따라 표면이 번들거립니다.

LED로 가면,

- 오전엔 맑은 하늘, 오후엔 노을 — **시간대별 콘텐츠 전환** 가능
- 브랜드 필름, 제품 컷, 이벤트 안내를 **같은 면에서 순환**
- **스스로 빛나기 때문에** 별도 배경 조명 불필요
- 철수 후 남는 폐기물 없음

부스 예산에서 LED는 비용 항목이 아니라 **출력물·조명·연출비를 흡수하는 항목**입니다.

#### **솔미디어랩은 단순 렌탈이 아니라 브랜드의 이미지를 보여주는 공간을 만들어 드립니다.**
