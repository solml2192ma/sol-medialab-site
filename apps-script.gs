 ...
 * 9) 메뉴에서 삽입 > 체크박스 클릭 (또는 데이터 > 데이터 확인 > 기준: 체크박스)
 *    → 이렇게 해두면 새 문의가 접수될 때마다 G열에 미체크 상태의 체크박스가 자동으로 생기고,
 *      답변을 마친 문의는 시트에서 직접 체크박스를 클릭해 "답변완료"로 표시하면 됩니다.
